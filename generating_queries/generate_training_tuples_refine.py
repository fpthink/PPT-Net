...
# after sort: business_run[0,4] residential_run[5,9] university_run[10,14]

folders = []
index_list = range(5, 15)   # residential_run[5,9] + university_run[10,14]
...
for folder in folders:
    ...
    # inhouse_datasets/residential_runX/pointcloud_25m_10/XXXX.bin
    df_locations['timestamp'] = runs_folder + folder + pointcloud_fols + df_locations['timestamp'].astype(str)+'.bin'
    df_locations = df_locations.rename(columns = {'timestamp': 'file'})    # rename: timestamp -> file
...
for folder in folders:
    df_locations = pd.read_csv(os.path.join(base_path, runs_folder, folder, filename), sep=',')
    # oxford/XXXXX/pointcloud_20m_10overlap/XXXXX.bin
    df_locations['timestamp'] = runs_folder + folder + pointcloud_fols + df_locations['timestamp'].astype(str)+'.bin'
...
construct_query_dict(df_train, "training_queries_refine_v1.pickle")
